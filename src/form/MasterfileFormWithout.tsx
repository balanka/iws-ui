...
export const MasterfileFormWithout = ({
  ...
  { fieldName: string; propertyName: string; accData: IMasterfile[] } & { children?: React.ReactNode }  // ← add this
): JSX.Element =>{
  ...
  return (
    <div style={{ ...styles.outer, paddingBottom: 10, width: '100%' }}>
      <CInputGroup style={{ height, ...rm }}>
        ...
        <CCol sm="4"><DatePickerField fieldName="changedate" label={t('common.changedate')} selected={current.enterdate} current={current} setCurrent={setCurrent} disabled={true}/></CCol>
      </CInputGroup>
      ...
      <CInputGroup style={{ height, ...rm }}>
        ...
        <CCol sm="4"><DatePickerField fieldName="postingdate" label={t('common.postingdate')} selected={current.changedate} current={current} setCurrent={setCurrent} disabled={true}/></CCol>
      </CInputGroup>
      {hasProp && (
      <CInputGroup style={{ height: hasProp ? height : 'auto', minHeight: hasProp ? 'auto' : height, ...rm }}>
        ...
        <CCol sm="4">
          {hasProp && <FormMasterfileXComboBox fieldName={propertyName} current={current} setCurrent={setCurrent} data={accData} defaultValue={initAcc} zIndex={11} disable={disable} styles={s} fontSize={12} />}
        </CCol>
      </CInputGroup>
      )}
      {children && (
