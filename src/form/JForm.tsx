...
const JForm = () => {
  ...
  const onRowSelected = (event: RowSelectedEvent) => setCurrent((event.data instanceof Array)?event.data[0]:event.data)
    const format = (d:IPeriodicAccountBalance) => {
        return {
            ...d
            , idebit: Number(d.idebit.toFixed(2))
  ...
  const sumData = (data:IPeriodicAccountBalance[]) => {
      const idebit = data.reduce((accumulator, currentData: IPeriodicAccountBalance) => accumulator + currentData.idebit, 0.0)
      const icredit = data.reduce((accumulator, currentData: IPeriodicAccountBalance) => accumulator + currentData.icredit, 0.0)
      const debit = data.reduce((accumulator, currentData: IPeriodicAccountBalance) => accumulator + currentData.debit, 0.0)
      const credit = data.reduce((accumulator, currentData: IPeriodicAccountBalance) => accumulator + currentData.credit, 0.0)
      const bdebit = data.reduce((accumulator, currentData: IPeriodicAccountBalance) => accumulator + currentData?.bdebit, 0.0)
      const bcredit = data.reduce((accumulator, currentData: IPeriodicAccountBalance) => accumulator + currentData?.bcredit, 0.0)
      ...
      return {idebit, icredit, debit, credit, bdebit, bcredit, currency, modelidx, companyx};
  }

  const { idebit, icredit, debit, credit, bdebit, bcredit, currency, modelidx, companyx} = sumData(rowData)
    ...
    const totalPac0:IPeriodicAccountBalance = {
        ...
        , bcredit:Number(0.0.toFixed(2)), account: '', modelid:modelidx, period:0, currency:currency, company: companyx}

    const totalPac:IPeriodicAccountBalance = {
        id: '-1', name: 'Total', idebit: Number(idebit.toFixed(2))
   ...
   // let currency = profile?.currency ?? 'XOF'
    ...
    console.log('totalPac', totalPac)
    if (current.toPeriod == undefined || current.toPeriod ==-1) {
        rowData.splice(0, rowData.length)
        rowData.push(totalPac0)
    } else {
        const index = rowData.findIndex(d => d.period === 0)
        if (index === -1 ) {
            rowData.push(totalPac)
        }
    }
    return (
