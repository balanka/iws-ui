...
const Edit = (url, token, record, data,  setCurrent) => {
  ...
  var result
  axios.put(url_, record, {headers: {'Authorization': `Bearer ${token}`}})
    .then(response => {
 ...
 const Add = (url, token, record, data, setCurrent) => {
   ...
   axios.post(url_, record, {headers: {'Authorization': `Bearer ${token}`}})
     .then(response => {
       const resp = response.data
...
const importFn =(str)=> React.lazy(() => import(`./${str}`))
 const Login = (history, url, data, setProfile, t, locale, currency, setMenu, setModule, setRoutes) => {
  const url_ = SERVER_URL.concat(url)
   console.log(' url', url)
     ...
     .then(response => {
       ...
       axios.get(moduleURL, {headers: {'Authorization': `Bearer ${token}`}})
         .then(response => {
           ...
           const routes_t = module_.map(e=> {return {...e, component:e.description, element: e.description?importFn(e.description):undefined}})
           //const routes_ = module_.map(e=> {return {...e, component: importFn(e.description)}})
           console.log('module_', module_)
           console.log('routes_t', routes_t)
 ...
 const Get = (url, token, history, func) => {
   const url_ = SERVER_URL.concat(url)
   let result
   axios.get(url_, {headers: {'Authorization': `Bearer ${token}`}})
     .then(response => {
       const resp = response.data
