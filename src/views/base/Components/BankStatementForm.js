...
function internal(url, token, history, initialState, data, setData,  current, setCurrent,  title, state
    , toggle, toggleToolbar, modelid_, t, toolbar, columns, rows, setSelectedRows, module_, locale, currency, company) {
    ...
    const submitPost = event => {
        event.preventDefault()
        const url_= module_.state3.concat('/post/').concat(current.company).concat('/').concat(rows.join(','))
  ...
  const reload = ()=> {
    ...
    const importData = ()=> {
      const url_= module_.ctx.concat('/').concat(current.path).concat('/').concat(current.header).concat('/').concat(current.char)
        .concat('/').concat(current.extension).concat('/').concat(company)
        ...
        return <>
            <Grid container spacing={2} style={{...styles.outer}} direction='column'>
                <BSFormHead styles={styles} title={title} collapse={state.collapse} setData={setData}
                            url={url} cancelEdit={cancelEdit} submitEdit={submitEdit} reload={reload} importData={importData}
                            submitQuery={submitQuery} submitPost={submitPost} toggle={toggle}
                            toggleToolbar={toggleToolbar} current={current}/>
                <FormFactory formid={modelid_} current={current} setCurrent={setCurrent} t={t} locale ={locale} currency ={currency}
                             collapse={state.collapse} height = {35}  styles={styles}/>
...
const BankStatementForm = () => {
    ...
    return internal(url, token, navigate,initialState, data, setData,  current || current_, setCurrent,  title, state
        , toggle, toggleToolbar, modelid_, t, toolbar, columns, rows, setSelectedRows, module_, locale, currency, company)
