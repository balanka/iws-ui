...
const JForm = () => {
   ...
   if ((typeof module_ === 'undefined') ||!module_ || module_.id === '11111') return navigate('/login')
    ...
    return Internal(isDebit, t, modelid, selected===MASTERFILE.pac?[ALL].concat(accData_):accData_, accUrl, token, navigate, setAccData, initAcc, current, getUrl, setData
      , initialState, setIsDebit, title, state, url, toggle, toggleToolbar, setCurrent, toolbar, data, columnsX)
